...
export const saveToDownloadHistory = async (jobTitle: string | undefined, score: number, fileName: string, analysisData?: AnalysisData) => {
  // Import dynamically to avoid circular deps
  const { saveDownloadToHistory } = await import('@/hooks/useDownloadHistory');
  ...
  saveDownloadToHistory({
    ...
    score,
  }).catch(err => console.warn('Failed to save download to Supabase:', err));
